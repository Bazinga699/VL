...
from lavis.models.beit import utils
from lavis.models.beit.modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
...
@register_model
def beit3_base_patch16_224_itc(pretrained=False, **kwargs):
    args = _get_base_config(**kwargs)
    args.normalize_output = False
    model = BEiT3(args, num_classes=1000, **kwargs)
    if pretrained:
        print('load pretrained image_encoder from: /data/pretrained_checkpoint/beit3_base_itc_patch16_224.pth')
        checkpoint = torch.load('/data/pretrained_checkpoint/beit3_base_itc_patch16_224.pth',map_location='cpu')
        msg = model.load_state_dict(checkpoint['model'], strict=False)
        #print(msg)
    return model

@register_model
def beit3_large_patch16_224_itc(pretrained=False, **kwargs):
    args = _get_large_config(**kwargs)
    args.normalize_output = False
    model = BEiT3(args, num_classes=1000, **kwargs)
    if pretrained:
        print('load pretrained image_encoder from: /data/pretrained_checkpoint/beit3_large_itc_patch16_224.pth')
        checkpoint = torch.load('/data/pretrained_checkpoint/beit3_large_itc_patch16_224.pth',map_location='cpu')
        msg = model.load_state_dict(checkpoint['model'], strict=False)
        #print(msg)
    return model
...
def get_pretrained_beit3_base():
    model = create_model('beit3_base_patch16_224_itc', pretrained=True)
    #convert_weights_to_fp16(model)
    return model

def get_pretrained_beit3_large():
    model = create_model('beit3_large_patch16_224_itc', pretrained=True)
    #convert_weights_to_fp16(model)
    return model
